...
def get_monthly_value(model_in, site_name, var_name, IGBP=None, clim_type=None):
    ...
    ntime  = len(time)
    time_intervals = np.diff(time)[0]/timedelta(seconds=3600) # units hour
    day_tot_step   = int(24/time_intervals)
    per_second_to_per_time_step = time_intervals*3600.
    ...
    var_in['Precip']   = f_met.variables['Precip'][:,0,0].data * per_second_to_per_time_step
    ...
    if var_name in ['NEE','GPP']:
        var_in.loc[:,var_name] = var_in[var_name][:]
    
    var_mean  = var_in.groupby(['year','month'])
    
    var_mean = var_in.groupby(['year', 'month']).agg({
    ...
    directory = f'./txt/{var_name}_monthly_value'
    check_directory_exist(directory)
    var_mean.to_csv(f'{directory}/{var_name}_monthly_value_{model_in}_{site_name}.csv')
    
    return var_mean

def save_monthly_value_parallal(var_name, model_in):
    ...
    with mp.Pool(mp.cpu_count()) as pool:
        pool.starmap(get_monthly_value, args_list)
    ...
    for site_name in remain_sites:
        try:
            var_mean = pd.read_csv(f'./txt/{var_name}_monthly_value/{var_name}_monthly_value_{model_in}_{site_name}.csv')
    ...
    directory = f'./txt/{var_name}_monthly_value'
    check_directory_exist(directory)
    var_means.to_csv(f'{directory}/{var_name}_monthly_value_{model_in}.csv')
...
if __name__ == "__main__":
    ...
    for model_in in model_list:
        ...
        save_monthly_value_parallal(var_name, model_in)

'''
History:
1. 22 July 2024, "cp ../LSM_VPD_PLUMBER2/process_step1_write_all_sites.py ."
...
def write_spatial_land_days(var_name, site_names, PLUMBER2_path, PLUMBER2_met_path, add_LAI=False,
    ...
    for i, site_name in enumerate(site_names):
        ...
        if i == 0:
            ...
            # var_output = var_output.append(var_output_tmp, ignore_index=True) # AttributeError: 'DataFrame' object has no attribute 'append'. Did you mean: '_append'?
            var_output = pd.concat([var_output, var_output_tmp], ignore_index=True)
    ...
    if add_LAI:
        if country_code !=None:
            var_output.to_csv(f'./txt/process1_{var_name}_all_sites_with_LAI_'+country_code+'.csv') # , mode='a', index=False
        else:
            var_output.to_csv(f'./txt/process1_{var_name}_all_sites_with_LAI.csv') # , mode='a', index=False
    else:
        if country_code !=None:
            var_output.to_csv(f'./txt/process1_{var_name}_all_sites_'+country_code+'.csv') # , mode='a', index=False
        else:
            var_output.to_csv(f'./txt/process1_{var_name}_all_sites.csv') # , mode='a', index=False

def add_obs_LAI_to_write_spatial_land_days(var_name, site_names, PLUMBER2_met_path):

    var_output   = pd.read_csv(f'./txt/process1_{var_name}_all_sites.csv',usecols=['time','month',
    ...
    var_output.to_csv(f'./txt/process1_LAI_all_sites.csv')
    return

def add_model_LAI_to_write_spatial_land_days(site_names, models_calc_LAI, model_LAI_names, PLUMBER2_path_input):

    var_output   = pd.read_csv(f'./txt/process1_LAI_all_sites.csv',na_values=[''])
    ...
    for i, site_name in enumerate(site_names):
        ...
        for model_with_LAI in models_calc_LAI:
            ...
            if np.all(np.isnan(LAI_tmp)):
                print('model name is ',model_with_LAI, 'site is ', site_name, 'np.all(np.isnan(LAI_tmp))=True')
            else:
                ...
                if model_ntime == ntime:
                    ...
                    print('model ', model_with_LAI, ' is hourly, model_ntime is', model_ntime, ' ntime is', ntime)
...
def add_model_LAI_to_write_spatial_land_days_parallel(site_names, models_calc_LAI):
    ...
    for site_name in site_names:
        ...
        var_LAI   = pd.read_csv(f'./txt/process1_LAI/model_LAI_{site_name}.csv', na_values=[''])
    ...
    var_output.to_csv(f'./txt/process1_LAI_all_sites_parallel.csv')
...
def add_model_SMtopXm_to_write_spatial_land_days(var_name, site_names, PLUMBER2_path, top_thickness=1.):
    ...
    var_output.to_csv('./txt/process1_SMtop'+str(top_thickness)+'m_all_sites.csv')
    return

def add_normalized_SMtopXm_to_write_spatial_land_days(var_name, site_names, PLUMBER2_path, top_thickness=1.):

    # read the variables
    var_output   = pd.read_csv(f'./txt/process1_SMtop'+str(top_thickness)+'m_all_sites.csv',
                               na_values=[''])
    ...
    var_output.to_csv('./txt/process1_normalized_SMtop'+str(top_thickness)+'m_all_sites.csv')
    return

...
def make_nc_file(PLUMBER2_path, var_name_dict, model_names, site_name, output_file, varname, zscore_threshold=2):
    ...
    for j, model_name in enumerate(model_names):
        ...
        if var_name_dict[model_name] != 'None':
            ...
            Var_tmp = f.variables[var_name_tmp][:].data # add .data to read data only
    ...
    for i, model_out_name in enumerate(model_out_names):
        ...
        if not os.path.exists(output_file):
            ...
            # set time dimensions
            if model_out_name == 'QUINCY':
                ntime               = len(model_times['CABLE'])
            ...
            if model_out_name == 'QUINCY':
                time.units          = model_time_units['CABLE']
                if model_time_calendars['CABLE'] != 'None':
                    time.calendar   = model_time_calendars['CABLE']
                time[:]             = model_times['CABLE']
            else:
            ...
            if model_out_name == 'QUINCY':
                var[:]          = model_vars[model_out_name][8760:]
            else:
            ...
            if var_name not in f.variables:
                ...
                if model_out_name == 'QUINCY':
                    f.variables[var_name][:]    = model_vars[model_out_name][8760:]
...
def add_SM_top1m_to_nc_file(PLUMBER2_path, output_file, site_name, SM_names, soil_thicknesses):
    ...
    for n, model_name in enumerate(model_names):
        ...
        if not file_path:
            ...
            # total soil layers thickness >1.0 m
            if np.sum(soil_thickness)>=1.0:
                ...
                # total soil layers thickness <1.0 m
                for i in np.arange(len(soil_thickness)):
                    if model_name == 'MuSICA' or model_name == 'STEMMUS-SCOPE':
                        # for the models with (time, lat, lon, nsoil)
                        if units_kg_m_2:
                            try:
                ...
                SM_tmp = SM_tmp/np.sum(soil_thickness)
